...
## 🔀 Мультисервер для одного пользователя

В текущей архитектуре у пользователя хранится только один `server_id`, а ключ строится из хоста этого сервера + `vpn_id`. Поэтому вариант с «переключением сервера кнопкой в ЛК» требует более серьёзной доработки БД, логики клиента и UI.

### Что проще реализовать

Самый быстрый путь — **вариант 1**: выдавать один саб-URL, который агрегирует конфиги с двух серверов.

Минимальный план:

1. Поднимать клиент (email / subId) с одинаковым `vpn_id` на обоих серверах.
2. Добавить сервис-агрегатор (endpoint вида `/sub/{vpn_id}`), который:
   - запрашивает подписки с обоих серверов,
   - объединяет строки конфигов,
   - удаляет дубликаты,
   - отдаёт единый ответ клиенту.
3. В боте вместо `server.host` отдавать домен агрегатора.

### Что нужно для кнопки «Сменить сервер»

Это уже **вариант 2** (сложнее):

- хранить привязку пользователя к нескольким серверам (новая таблица, например `user_servers`),
- создавать/обновлять клиента на каждом сервере или мигрировать клиента между серверами,
- добавлять кнопку выбора сервера в ЛК,
- гарантировать, что продление/промокоды/триал синхронно применяются к нужному серверу.

Если нужна самая быстрая и надёжная реализация — начинайте с варианта 1.

### Где endpoint и где код

- Endpoint агрегатора: `GET /sub/{vpn_id}`
- Регистрация route: `app/bot/routers/__init__.py`
- Handler агрегатора: `app/bot/routers/download/handler.py` (`multiserver_subscription`)
- Генерация ссылки, которую бот отдаёт пользователю: `app/bot/services/vpn.py` (`get_key`)

### Нужен ли отдельный саб-сервер

Отдельный сервис под саб-сервер **не нужен**: endpoint встроен в этот же бот (aiohttp-приложение).

### Пошаговый запуск рабочей мультисерверной версии

1. **Подготовьте домен бота**
   - `BOT_DOMAIN` должен указывать на домен/URL, доступный пользователям.
   - Endpoint подписки будет доступен по адресу: `https://<BOT_DOMAIN>/sub/<vpn_id>`.

2. **Заполните `.env`**
   Минимально важно для мультисервера:
   - `BOT_DOMAIN`
   - `XUI_USERNAME`, `XUI_PASSWORD` (и при необходимости `XUI_TOKEN`)
   - `XUI_SUBSCRIPTION_PORT`, `XUI_SUBSCRIPTION_PATH`
...
4. **Добавьте VPN-серверы в пул бота**
   Делается в админке бота (раздел управления серверами): для каждого сервера укажите имя, host и лимит клиентов.
   > Эти host-адреса — это и есть «конфиги серверов» на уровне бота. Они хранятся в таблице `servers`.

5. **Синхронизируйте серверы**
   В админке нажмите «Sync servers», чтобы бот проверил и актуализировал пул.

6. **Создайте/продлите подписку пользователю**
   После покупки (или триала) бот выдаст пользователю ссылку формата `/sub/{vpn_id}` вместо ссылки одного сервера.

### Как привязать сабсервер к боту и серверам

- «Сабсервер» в этой реализации = встроенный endpoint `/sub/{vpn_id}` внутри бота.
- Привязка к серверам происходит автоматически через пул `servers` (все серверы с `online=true` участвуют в агрегации).
- Привязка к пользователю происходит по `vpn_id`: endpoint сначала проверяет пользователя в БД, затем собирает его конфиги со всех online-серверов.

### Как протестировать

1. Возьмите `vpn_id` пользователя (из БД `users.vpn_id`).
2. Проверьте endpoint напрямую:
   ```bash
   curl -v "https://<BOT_DOMAIN>/sub/<vpn_id>"
   ```
3. Декодируйте результат (должен быть base64 со списком конфигов построчно):
   ```bash
   curl -s "https://<BOT_DOMAIN>/sub/<vpn_id>" | base64 -d
   ```
4. Ожидаемое поведение:
   - `200` и непустой base64 — всё ок.
   - `404 Subscription not found.` — не найден пользователь с таким `vpn_id`.
   - `503 No online servers available.` — нет online серверов в пуле.
   - `404 No subscription configs found.` — серверы доступны, но отдали пустые/невалидные подписки
   - 
